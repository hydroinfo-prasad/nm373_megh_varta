...
def images_to_probs(net, images):
    ...
    return preds*std_value + mean_value
...
def calc_loss(pred, target, metrics, bce_weight=0.5):
    ...
    loss = mse_loss#perceptual_loss( pred[:,:,:984,:1074], target ) + 500*mse_loss
...
writer = SummaryWriter('/nfs/151/gpu/gunji/SIH/pytorch-unet/logs/tir_inp3_ds4_stand')
...
model,optimizer_ft, exp_lr_scheduler = train_model(model, optimizer_ft, exp_lr_scheduler, writer,num_epochs=100)

torch.save(model,"tir_inp3_ds4_stand.ckpt")
writer.close()
